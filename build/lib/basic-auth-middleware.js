...
exports.default = function (request, response, next) {
  if (!request.headers.authorization) {
    return next(new _httpErrors2.default(400, 'Invalid request - no information'));
  }

  var base64AuthHeader = request.headers.authorization.split(' ')[1];
  if (!base64AuthHeader) {
    return next(new _httpErrors2.default(400, 'Invalid request - improper information'));
  }
  ...
  if (!username || !password) {
    return next(new _httpErrors2.default(400, 'Invalid request - no username or no password'));
  }
